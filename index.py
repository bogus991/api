...
class XTB:
	...
	"arguments": {
        ...
        trades_live_json = json.dumps(trades_live)
        result = self.send(trades_live_json)
        result = json.loads(result)
        trades_live= result["returnData"]
        return trades_live



    def get_TickPrices(self, level, symbols, timestamp):
        ...
        tick_live_json = json.dumps(tick)
        result = self.send(tick_live_json)
        result = json.loads(result)
        tick= result["returnData"]
        return tick
    ...
    def make_Trade(self, symbol, cmd, transaction_type, volume, comment="", order=0, sl=0, tp=0, days=0, hours=0, minutes=0):
        ...
        price = (price[1]["open"]+price[1]["close"])/10
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        status, order_code = API.make_Trade('US500', 0, 0, volume=0.01, comment='1111')
